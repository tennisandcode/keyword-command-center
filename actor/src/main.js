...
await Actor.main(async () => {
  ...
  for (const { asin } of products) {
    ...
    // ---- Deep competitor X-ray on the top N keywords by opportunity score
    // (every run, so competitor data + listing links stay fresh) ----
    const targets = scored.slice(0, maxCompetitorLookups);
